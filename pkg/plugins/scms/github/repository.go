...
func searchRepositories(client GitHubClient, queryStr string, retry int, ctx context.Context) ([]string, error) {
	var allRepos []string
	...
	for {
		...
		if err != nil {
			if strings.Contains(err.Error(), ErrAPIRateLimitExceeded) && retry < MaxRetry {
				rateLimit, err := queryRateLimit(client, ctx)
				if err != nil {
				...
				if retry < MaxRetry {
					...
					rateLimit.Pause()
					return searchRepositories(client, queryStr, retry+1, ctx)
				}
		...
		for _, edge := range query.Search.Edges {
			...
			allRepos = append(allRepos, edge.Node.Repository.Name)
		}
